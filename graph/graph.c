...
Graph *initializeGraph(int directed, int simple) {
    ...
    graph->adjacencyMatrix = (float *)(&(graph->nodeArray[0]) + graph->size);
...
int addEdge(Graph *graph, Node *node1, Node *node2, float decoration) {
    if (!(graph->nodeArray[node1->label]) ||
        !(graph->nodeArray[node2->label])) {
        return 0;
    }
    ...
    if (!(graph->simple) || (node1 != node2)) {
        graph->adjacencyMatrix[node1->label * (graph->size) + node2->label] =
            decoration;
        if (!graph->directed) {
            graph
                ->adjacencyMatrix[node2->label * (graph->size) + node1->label] =
                decoration;
        }
        graph->edgeSum++;
    }
...
Graph *initializeRandGraph(int directed, int simple, int n, int m) {
    size_t maxPossible = (size_t)(directed * (n * n - simple * n)) +
                         !directed * (chooseTwo(n) + (size_t)(!simple * n));
        ...
        for (size_t i = 0; i < graph->order; i++) {
            for (size_t j = 0; j < graph->order; j++) {
                if (graph->adjacencyMatrix[i * graph->size + j] != 0.0) {
                    ...
                    addEdge(graph, graph->nodeArray[i], graph->nodeArray[j],
                            1.0);
                }
            }
        }
...
Graph *resizeGraph(Graph *graph) {
    size_t diff =
        4 * (((unsigned long)graph->size) * ((unsigned long)graph->size)) *
        sizeof(void *);
    ...
    newGraph->adjacencyMatrix =
        (float *)(&(newGraph->nodeArray[0]) + newGraph->size);
    for (int i = 0; i < ((int)graph->order); i++) {
        for (int j = 0; j < ((int)graph->order); j++) {
            newGraph->adjacencyMatrix[((size_t)i) * (newGraph->size) +
                                      ((size_t)j)] =
                graph->adjacencyMatrix[((size_t)i) * (graph->size) +
                                       ((size_t)j)];
        }
    }
